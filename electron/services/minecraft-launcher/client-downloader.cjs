...
const networkUtils = require('./network-utils.cjs');
const assetManager = require('./asset-manager.cjs');
const profileHandler = require('./profile-handler.cjs');
...
class ClientDownloader {
  ...
  async downloadJson(url, maxRetries = 3) {
    return networkUtils.downloadJson(url, maxRetries);
  }

  async _downloadJsonSingle(url) {
    return networkUtils.downloadJson(url, 1);
  }

  async downloadFile(url, filePath, maxRetries = 3, progressCallback = null) {
    return networkUtils.downloadFile(url, filePath, maxRetries, progressCallback);
  }

  async _downloadFileSingle(url, filePath, progressCallback = null) {
    return networkUtils.downloadFile(url, filePath, 1, progressCallback);
  }

  async installFabricLoader(clientPath, minecraftVersion, fabricVersion = "latest") {
    return profileHandler.installFabricLoader(this.javaManager, clientPath, minecraftVersion, fabricVersion, this.emitter);
  }
...
`;
  ...
  async clearAssets(clientPath) {
    return assetManager.clearAssets(clientPath);
  }

  async enrichFabricJson(clientPath, fabricProfileName) {
    return profileHandler.enrichFabricJson(clientPath, fabricProfileName);
  }

  async injectVanillaDownloads(clientPath, profileName, vanillaVersion) {
    return profileHandler.injectVanillaDownloads(clientPath, profileName, vanillaVersion);
  }
  ...
  async downloadAssets(clientPath, minecraftVersion) {
    return assetManager.downloadAssets(clientPath, minecraftVersion, this.emitter);
  }
}
